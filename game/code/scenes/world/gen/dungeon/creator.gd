...
var width = 18; var height = 18; #this array is the size of the map. it needs to be + 2 on each value to create a border
var grid = []; #base grid
var rand_grid = []; #used to randomly generate the grid (map)


func _ready(): #called when the node enters the scene for the first time.
	buildArray() #builds the array
	generateMiniMap() #generates the minimap
	generateHallways() #generates the hallways
	generateMap()
...
func generateMiniMap():
	for i in range(0, width): #builds a 18x18 mini map of void tiles. the extra 2x2 is for easier generation due to array restrictions.
		for j in range(0, height):
			grid[i][j] = 0
			setTileM(i, j, 2) #sets current tile to void
	...
	for i in range(1, width - 1): #this randomly generates the map/dungeon
		for j in range(1, height - 1):
			...
			if rand_grid[i][j] == 1: 
					setTileM(i, j, 0) #sets minimap to tile 0
...
func setTileM(var i, var j, var k):

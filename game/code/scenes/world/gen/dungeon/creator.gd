...
onready var dungeon = get_node("Navigation2D").get_node("Dungeon")
...
func _ready():
	...
	for i in range(1, width - 1): #this randomly generates the map/dungeon
		for j in range(1, height - 1):
			...
			if rand_grid[i][j] == 1: 
					tilemap.set_cell(i, j, 0) #sets minimap to tile 0
